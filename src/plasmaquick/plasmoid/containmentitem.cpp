...
void ContainmentItem::mimeTypeRetrieved(KIO::Job *job, const QString &mimetype)
{
    ...
    QPoint dropPoint = m_dropMenu->dropPoint();
    if (window()) {
        // containments want the drop point in relative coordinates
        dropPoint -= window()->position();
    }
    ...
    if (mimetype.isEmpty() && appletList.isEmpty()) {
        ...
        qCDebug(LOG_PLASMAQUICK) << "Received a suitable dropEvent at global" << m_dropMenu->dropPoint() << "local" << dropPoint;
        ...
        if ((!appletList.isEmpty() || !wallpaperList.isEmpty() || isPlasmaPackage) && !m_dropMenu->isMultipleMimetypes()) {
            ...
            if (isPlasmaPackage) {
                ...
                connect(installPlasmaPackageAction, &QAction::triggered, this, [this, packagePath, dropPoint]() {
                    using namespace KPackage;

                    PackageJob *job = PackageJob::update(QStringLiteral("Plasma/Applet"), packagePath);
                    connect(job, &KJob::finished, this, [this, packagePath, job, dropPoint]() {
                        ...
                        createApplet(package.metadata().pluginId(), QVariantList(), QRect(dropPoint, QSize(-1, -1)));
                    });
                });
            }
            ...
            for (const auto &info : std::as_const(appletList)) {
                ...
                connect(action, &QAction::triggered, this, [this, action, mimetype, url, dropPoint]() {
                    Plasma::Applet *applet = createApplet(action->data().toString(), QVariantList(), QRect(dropPoint, QSize(-1, -1)));
            ...
            Plasma::Applet *applet = createApplet(QStringLiteral("org.kde.plasma.icon"), QVariantList(), QRect(dropPoint, QSize(-1, -1)));
